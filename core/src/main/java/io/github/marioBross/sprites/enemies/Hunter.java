...
public class Hunter extends Enemy {
    ...
    public Hunter(PlayScreen screen, float x, float y) {
        ...
        for (int i = 0; i < 4; i++)
            frames.add(new TextureRegion(screen.getAtlas().findRegion("hunter-walk"), i * 32, 0, 32, 32));
        walkAnimation = new Animation<>(0.4f, frames);
        shell = new TextureRegion(screen.getAtlas().findRegion("turtle"), 64, 0, 16, 24);
        walkAnimation = new Animation<>(0.4f, frames);
        currentState = previousState = State.WALKING;
        stateTime = 0;
        setBounds(getX(), getY(), 32 / MarioBross.PPM, 32 / MarioBross.PPM);
    ...
    protected void defineEnemy() {
        ...
        shape.setAsBox(6 / MarioBross.PPM, 16 / MarioBross.PPM);
